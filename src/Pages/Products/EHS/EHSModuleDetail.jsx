...
export default function EHSModuleDetail() {
  const { id } = useParams();
  const navigate = useNavigate();
  const moduleInfo = EHSModules.find((m) => m.id === id);
  ...
  const currentIndex = EHSModules.findIndex(m => m.id === id);
  const nextModule = EHSModules[currentIndex + 1];
      ...
      >
        ...
        <div className="md-header-bar">
          ...
          <button
            ...
            onClick={() => {
              if (nextModule) {
                navigate(`/EHSModules/${nextModule.id}`);
              } else {
                navigate("/Products/EHS");

                setTimeout(() => {
                  const el = document.getElementById("EHS-core-modules");
                  if (el) {
