...
export default function CSRModuleDetail() {
  const { id } = useParams();
  const navigate = useNavigate();
  const moduleInfo = CSRModules.find((m) => m.id === id);
  ...
  const currentIndex = CSRModules.findIndex(m => m.id === id);
  const nextModule = CSRModules[currentIndex + 1];
  return (
      ...
      >
        ...
        <div className="md-header-bar">
          ...
          <button
            ...
            onClick={() => {
              if (nextModule) {
                navigate(`/CSRModules/${nextModule.id}`);
              } else {
