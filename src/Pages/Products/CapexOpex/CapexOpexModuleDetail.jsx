...
export default function CapexOpexModuleDetail() {
  const { id } = useParams();
  const navigate = useNavigate();
  const moduleInfo = CapexOpexModules.find((m) => m.id === id);
  ...
  const currentIndex = CapexOpexModules.findIndex(m => m.id === id);
  const nextModule = CapexOpexModules[currentIndex + 1];
      ...
      >
        ...
        <div className="md-header-bar">
          ...
          <button
            ...
            onClick={() => {
              if (nextModule) {
                navigate(`/CapexOpexModules/${nextModule.id}`);
              } else {
                navigate("/Products/ProcurementDecisionTool");

                setTimeout(() => {
                  const el = document.getElementById("CapexOpex-core-modules");
                  if (el) {
